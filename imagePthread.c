...
void* convolute_thread(void* arg) {
    ...
    int row, pix, bit;
    int span = data->srcImage->bpp * data->srcImage->bpp;
...
void convolute(Image* srcImage, Image* destImage, Matrix algorithm) {
    ...
    for (int t = 0; t < NUM_THREADS; t++) {
        ...
        args[t].end_row = (t == NUM_THREADS - 1) ? srcImage->height: (t + 1) * rows_per_thread;

        pthread_create(&threads[t], NULL, convolute_thread, &args[t]);
    }
